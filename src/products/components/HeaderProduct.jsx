...
export const HeaderProduct = () => {
  ...
  const getProductRate=async ()=>{
    const queryRates = query(collection(FirebaseDB, 'products'), orderBy('rate', 'desc'),limit(1));
      ...
      {maxRate.map((products) => (
         <div key={products.id}  className="md:flex max-w-3xl mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-4xl m-3">
            ...
            <div className="p-8 ml-4">
                <div className="uppercase tracking-wide text-sm text-indigo-500 font-semibold">{ products.name }</div>
                  <a className="block mt-1 text-lg leading-tight font-medium text-black hover:underline" href="#" >{ products.category}</a>
                  <p className="mt-2 text-gray-500">{ products.description}</p>
                  <div className="flex items-center">
                    <p className="tracking-wide text-sm text-violet-900 font-semibold">Higher Rate { products.rate } </p>
